	...
	applesearch.init = function ()
	{
		...
		if(srchform) {
			...
			if ( navigator.userAgent.toLowerCase().indexOf('safari') < 0  && document.getElementById ) {
				...
				if (dummy)	dummy.href = "inc/applesearch.css";
				...
				if(srchinput) {
					// set the placeholder text based off the label, when no value supplied
					if(srchinput.value=='' || srchinput.value==labelText) {
						...
						srchinput.onblur = function () {
								if (this.value == '') {
									...
									if (typeof(window['labelColor']) != "undefined") {this.style.color = labelColor};
								}
							}
				}
